...
class DBOperations:
    ...
    def create_table_db(self, db_name, column_names):
        ...
        try:
            # logging
            file_db_table_create_log = open('Training_Logs/DbTableCreateLog.txt', "a+")
            log_msg = f"""Entered 'create_table_db' method of 'DBOperations' class."""
            self.logger.log(file_db_table_create_log, log_msg)

            file_db_connection_log = open('Training_Logs/DataBaseConnectionLog.txt', "a+")
            log_msg = f"""Entered 'create_table_db' method of 'DBOperations' class."""
    ...
    def my_method(self, db_name):
        ...
        for file in onlyfiles:
            try:
                file_path = os.path.join(good_file_path, file)
        ...
        new_conn = self.db_connection(db_name)
